<template>
    <div class="font-Karantina text-2xl font-bold space-x-6">
        ...
        <canvas id="myLineChartsss"></canvas>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        createChart(data) {
            ...
            const ctx = document.getElementById('myLineChartsss').getContext('2d');
        ...
        createDailyChart(ctx, data) {
            ...
            data.forEach(relawan => {
                const date = new Date(relawan.tanggal).toString().split('T')[0];
                if (!dailyData[date]) {
                    dailyData[date] = 0;
                }
                dailyData[date]++;
